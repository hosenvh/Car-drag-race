...
public class CarStatsCalculatorEditor : MonoBehaviour
{
    ...
    public event FinishedCalculatingNewPPIndex NewPPIndexCalculated;
    public event Action<UpgradeScreenCarStats> CarStatsUpdated;


    public static CarStatsCalculatorEditor Instance
    {
        get;
        private set;
    }
    ...
    private void Awake()
    {
        CarStatsCalculatorEditor.Instance = this;
    }
    ...
    public void CalculateUpgradeScreenPerformanceIndex(CarUpgradeSetup upgradeSetup,bool ignoreTierChanged)
    {
        m_currentUpgradeSetup = upgradeSetup;
        this.CalculatePerformanceIndexWorder(upgradeSetup,this.completeCalcCurrentLevel, this.completeCalcWantedLevel, ignoreTierChanged);
    }

    public void CalculateSportsPackScreenPerformanceIndex()
    {
        this.CalculatePerformanceIndexWorder(null,0, 0,true);
    }

    private void CalculatePerformanceIndexWorder(CarUpgradeSetup upgradeSetup,int completeCalcCurrentLevel, int completeCalcWantedLevel
    ,bool ignoreTierChanged)
    {
        this.PerformCalculateStatsForUpgradeScreen(upgradeSetup, this.completedCalcUpgradeType, completeCalcCurrentLevel, completeCalcWantedLevel);
        OptimalGearChangeSpeedCalculator optimalGearChangeSpeedCalculator = new OptimalGearChangeSpeedCalculator(this.carPhysics);
        optimalGearChangeSpeedCalculator.CalculateGearChangeSpeeds();
        this.statsDict.Clear();
        this.CalculateNewPerformanceIndex(false, this.carPhysics, 0f, ignoreTierChanged);
    }
    ...
    public void Update()
    {
        if (this.IsCalculatingPerformance)
        {
            ...
            if (eState == CarPerformanceIndexCalculator.eState.FINISHED)
            {
                ...
                {
                    //PlayerProfileManager.Instance.ActiveProfile.PlayerPhysicsSetup = CarStatsCalculatorEditor.Instance.playerCarPhysicsSetup;
                    CarStatsCalculatorEditor.Instance.playerCarPhysicsSetup.SetPerformancePotentialStats(this.piCalculator, true);
                    CarStatsCalculatorEditor.Instance.SetOutStats(eCarStatsType.PLAYER_SETUP_CAR);
                }
...
//                    : this.piCalculator.PerformanceIndex;
                ...
                {
                    this.NewPPIndexCalculated(this.playerCarUpgradeScreenSetup.carInfo.MediumName, this.piCalculator.CarTierEnum,
                        performanceIndex, this.piCalculator.QuarterMileTime);
                    //PlayerProfileManager.CurrentCarNewPP(this.playerCarUpgradeScreenSetup.carInfo.MediumName, this.piCalculator.CarTierEnum, this.piCalculator.PerformanceIndex, this.piCalculator.QuarterMileTime);
                }
                if (this.hasCachedCalculationToPerform)
                {
                    //PlayerProfile activeProfile = PlayerProfileManager.Instance.ActiveProfile;
                    this.CalculateStatsForUpgradeScreen(m_currentUpgradeSetup, this.cachedUpgradeType, this.cachedCurrentLevel, this.cachedToLevel);
    ...
    public void CalculateStatsForUpgradeScreen(CarUpgradeSetup zCurrentSetup, eUpgradeType zUpgradeType,
        int zCurrentUpgradeLevel, int zToUpgradeLevel)
    {
        this.CreatePhysicsIfNeeded();
        if (this.IsCalculatingPerformance)
        {
        ...
        CarStatKey carStatKey = new CarStatKey((int)zCurrentSetup.UpgradeStatus[eUpgradeType.BODY].levelFitted,
            (int)zCurrentSetup.UpgradeStatus[eUpgradeType.TURBO].levelFitted,
            (int)zCurrentSetup.UpgradeStatus[eUpgradeType.ENGINE].levelFitted,
            (int)zCurrentSetup.UpgradeStatus[eUpgradeType.TRANSMISSION].levelFitted,
            (int)zCurrentSetup.UpgradeStatus[eUpgradeType.INTAKE].levelFitted,
            (int)zCurrentSetup.UpgradeStatus[eUpgradeType.NITROUS].levelFitted,
            (int)zCurrentSetup.UpgradeStatus[eUpgradeType.TYRES].levelFitted, zToUpgradeLevel, zUpgradeType,
            zToUpgradeLevel > zCurrentUpgradeLevel);
        ...
        //if (flag)
        //{
        //    if (CarStatsUpdated != null)
        //    {
        //        CarStatsUpdated(stats);
        //    }
    ...
    private void UpdateUIAndStoreInCache(CarStatKey carStatHash)
    {
        ...
        if (CarStatsUpdated != null)
        {
            CarStatsUpdated(upgradeScreenCarStats);
        }
    }
