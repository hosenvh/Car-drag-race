...
public class WebRequestQueueWorkItem
{
	...
	public WebRequestQueueWorkItem(string zFunctionName, string zDesc, JsonDict zPostdata, WebClientDelegate2 zCallback, object zUserData = null, string zHashString = "", JsonDict zHeaders = null, int zMaxRetries = 5)
	{
	...
	public bool HasEmptyTokens()
	{
		return (this.postdata.Exists("ryft") && this.postdata.GetString("ryft") == string.Empty) || (this.postdata.Exists("mpt") && this.postdata.GetString("mpt") == string.Empty);
	}
	...
	public void InjectTokens()
	{
		if (this.postdata.Exists("ryft"))
		{
			this.postdata.Set("ryft", UserManager.Instance.currentAccount.RYFToken);
		}
		if (this.postdata.Exists("mpt"))
		{
			this.postdata.Set("mpt", UserManager.Instance.currentAccount.MPToken);
		}
	}
	...
	public void StartCall()
	{
		...
		{
			this.webRequest = UserManager.Instance.webClient.ServerFunction(this.functionName, this.postdata);
		}
		else
		{
			this.webRequest = UserManager.Instance.webClient.StartFunctionCall(this.functionName, this.postdata, this.hashString);
		}
	...
	{
		this.ReleaseWebRequest();
		this.webRequest = UserManager.Instance.webClient.StartFunctionCallWithHeadersAndURL(this.functionName, this.headers, this.postdata, this.hashString);
        this.lastRequestTime = GTDateTime.Now;
	}
